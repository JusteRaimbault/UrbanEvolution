...
import urbanevolution.Matrix.MatrixImplementation
...
object UrbanEvolution {
  ...
  case class InnovationResult(
    ...
    def averageDiversity: Double = {
      ...
      val normPop =populations%*%DenseMatrix.diagonal(populations.colSum.map(1/_))
      innovationShares.map(m => (normPop*m).map(x => x*x).colSum).reduceLeft(arraySum).map(1 - _).sum / normPop.ncols
    }
  ...
  def apply(populationFile: String,
           ...
           )(implicit m: MatrixImplementation = Matrix.defaultImplementation, rng: Random): UrbanEvolution = {
    ...
    val dates = CSV.readCSV(datesFile,withHeader=false).values.head.map(_.toDouble).toArray

    UrbanEvolution(populationMatrix,distancesMatrix,dates,rng,growthRate,innovationWeight,gravityDecay,innovationDecay,
  ...
  def legacyNewInnovationShares(previousInnovShares: Seq[Double],currentPopulations: Seq[Double], newInnovationHierarchy: Double, earlyAdoptersRate: Double)(implicit rng: Random): (Seq[Double], Seq[Double]) = {
    ...
    log("Innovative city : "+innovativeCityIndex)
  ...
  def mutationInnovation(population: Seq[Double],
    ...
    if (innovativeCities.nonEmpty) log("Innovative cities: "+innovativeCities)
    ...
    if (innovativeCities.isEmpty) (false, Seq.empty[Double], Seq.empty[Seq[Double]])
      ...
      log("New utilities = "+newUtilities)
  ...
  /**
    ...
    */
  def run(model: UrbanEvolution): InnovationResult  = {

    log("Running "+model.toString)
    ...
    for (t <- 1 until p) {
      ...
      log("Macro levels = "+macroAdoptionLevels.toSeq)
